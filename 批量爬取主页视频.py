...
a = input('请粘贴用户主页地址\n')
a = requests.get(url=a, headers=headers).url
...
while 1:
    ...
    if max_cursor == 0:  # 当max_cursor重新变为0时意味着页面已全部加载完
        break
print('共解析到' + str(len(video_name)) + '个视频\n')

count = 0
for (name, url) in zip(video_name, video_url):
    count += 1
...
print(f'{count}个视频全部下载完成!!\t共花费 {spend_time} 秒')
